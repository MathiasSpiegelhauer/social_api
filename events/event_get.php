...
if (isset($REQ['token'])) {
    $event = fetch_assoc("SELECT title,token, description, address, longitude, latitude, creator_id FROM events where token = ?", [$REQ['token']]) ?? false;
    ...
    if (isset($REQ['lat']) && isset($REQ['long'])) {
        ...
        $events = fetch_all("SELECT id,token, title, description, address, longitude, latitude, creator_id FROM events", []);
    }
